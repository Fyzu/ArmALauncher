...
void repoEdit::recieveData(Repository repo, int currentRow, bool newRepo) {
    ...
    qDebug() << "Debug-repoEdit: Recieve start";
    newR = newRepo;
    Row = currentRow;

    if(newRepo) {
        ui->repoName->setText("Новый репозиторий");
    ...
    this->open();
    qDebug() << "Debug-repoEdit: Recieve succ";
}

void repoEdit::on_repoType_currentIndexChanged(int index)
{
    if(index == 0)
        ui->label->setText("Url сервера Yoma Addon Sync должен заканчиваться на .7z и начинаться с http:// или ftp://");
    else
        ui->label->setText("Url сервера ArmA3Sync должен заканчиваться на .a3s/autoconfig и начинаться с http:// или ftp://");
}
...
void repoEdit::on_saveButton_clicked()
{
    ...
    qDebug() << "Debug-repoEdit: Send start";
    Repository repo;
    ...
    this->close();
    qDebug() << "Debug-repoEdit: Send succ";
}
